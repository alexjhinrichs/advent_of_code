...
def part2(input)
    ...
    starting_keys = []
    nodes.keys.each do |y|
        if y.to_s[-1] == "A"
            starting_keys.push(y)
        end
    end
    attempts_to_get_to_z = []
    directions = directions * 10000

    starting_keys.each do |x|
        count = 0
        directions.chars.each do |y|
            y = y.to_sym
            key = nodes[x][y].to_sym
            x = key
            count = count + 1
            if x.to_s[-1] == 'Z'
                attempts_to_get_to_z.push(count)
                break
            end
        end
    end
    p attempts_to_get_to_z.reduce(1, :lcm)
end
