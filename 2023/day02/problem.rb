...
# array = input.split("\n").map! { |game| 
#     sets = game.split(": ")[1].split("; ").map { |set| 
#     pulls = set.split(',')
#     pulls.all? do |pull|
#         count, color = pull.split(' ')
#         limits[color] >= count.to_i
#     end
...
array = part2.split("\n").map! { |game|
    ...
    sets = game.split(": ")[1].split("; ").each { |set| 
    pulls = set.split(',')
    pulls.each do |pull|
        count, color = pull.split(' ')
        if maxCount[color] < count.to_i
            maxCount[color] = count.to_i
        end
    end
}
maxCount['red'] * maxCount['green'] * maxCount['blue']
}.inject(:+)
